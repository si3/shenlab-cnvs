...
# It uses bedtools (make sure it's installed properly)
...
# Pipeline - continues pipeline by invoking Run_RD_reformatters
...
usage="
	...
	using bedtools to get absolute RD per interval, needs a ref file per batch run
...
TmpLog="RD_calculator_2_"$LogNam
...
StepCmd="bedtools multicov -bams $BamFil -bed $WindFil -q $Min_MQ > $COV_DIR/$BamNam.coverage.bed" 
#awk '{print $1, $2, $3, $NF, $NF/($3-$2)}' > $COV_DIR/$BamNam.coverage.bed"
funcRunStep

if [[ $ArrNum == $NoSamples ]]; then # Checks end job, does housekeeping
	echo "Parallel is done" >> $TmpLog
	#mkdir $COV_DIR/mosdepth
	mkdir $COV_DIR/bedtools
	...
	mv $COV_DIR/*.coverage.bed $COV_DIR/bedtools 
	echo "Directory bedtools created in "$COV_DIR >> $TmpLog
